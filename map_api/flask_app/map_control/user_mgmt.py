...
class User(UserMixin) :
    ...
    def __init__(self, user_id, web_id, user_pw) :
        self.id = user_id  # self.id attributor는 flask_login에서 지원하기 때문에 변경시 에러남
        self.web_id = web_id
    ...
    def get(user_id) :  
        ...
        user = User(user_id=user[0], web_id=user[1], user_pw = [2])   # DB 컬럼 순서. Line22 에서 가져오는것
        return user
    ...
    def get(web_id) :  
        ...
        user = User(user_id=user[0], web_id=user[1], user_pw = [2])   
        return user
    
    # 새로운 이용자 저장 , 중복된 web_id가 mysql에 들어가지 않도록 함
    @staticmethod
    def create(web_id, user_pw) :
        user = User.find(web_id)          # 이용자 검색해보고 
        if user == None :                 # 이용자가 없을 경우    
            mysql_db = conn_mysqldb()        
            db_cursor = mysql_db.cursor()   
            sql = "INSERT INTO user_info (WEB_ID, USER_PW) VALUES ('%s', '%s') " % (str(web_id), str(user_pw)) 
            db_cursor.execute(sql)
